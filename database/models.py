...
class Database:
    ...
    async def insert_tasks(self, user_id, tasks=None, planning_date=None, done=False):
        ...
        for task in tasks:
            ...
            VALUES (?, ?, ?, ?);'''
            await self._execute_query(insert_query, (user_id, task, planning_date, done))
        logging.info(f"Tasks for user {user_id} added")

    async def select_tasks(self, user_id, plan_date=date.today() + timedelta(days=1)):
        select_query = '''
        ...
        ORDER BY task;
        '''
        record = await self._execute_query(select_query, (user_id, plan_date), select=True)
        return record
    ...
    async def update_tasks(self, user_id, task_id, field, value):
        ...
        '''
        await self._execute_query(update_query, (field, value, task_id, user_id))
